...
4. Inside the `apps/cli` directory run `yarn archive` to generate the standalone executable used by the `menu-bar`
5. Finally, run `yarn update-cli` inside the `apps/menu-bar` directory run to update the local cli file
